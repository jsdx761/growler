...
public class AircraftsFetchTask implements Runnable {
  ...
  public void run() {
    ...
    if(Configuration.DEBUG_INJECT_TEST_AIRCRAFTS != 0) {
      ...
      try {
        ...
        connection.setReadTimeout(Configuration.AIRCRAFTS_CONNECT_TIMEOUT);

        if(mUser.length() != 0 && mPassword.length() != 0) {
          if(!url.getProtocol().equals("https")) {
            Log.w(TAG, "Refusing to send credentials over non-HTTPS connection");
          }
          else {
    ...
    List<Alert> aircrafts = parseAircrafts(json, mLocation, mAircraftsDatabase.getInterestingAircrafts());
    if(aircrafts == null) {
      onDone(null);
      return;
    }
  ...
  /**
   * Parse the OpenSky JSON response into a list of Alert objects, filtering
   * by interesting aircraft, ground status, and distance.
   */
  static List<Alert> parseAircrafts(JSONObject json, Location location,
                                     Map<String, String[]> interestingAircrafts) {
    ...
    try {
      ...
      if(jsonAircrafts != null) {
        ...
        for(int i = 0; i < jsonAircrafts.length(); i++) {
          ...
          try {
            // Only keep aircraft state vectors of relevant types, not on the
            // ground and within the configured distance
            String transponder = jsonAircraft.getString(Alert.OPENSKY_ICAO24);
            Log.i(TAG, String.format("aircraft transponder icao24 address %s", transponder));

            String[] aircraftInfo = interestingAircrafts.get(transponder);
            if(aircraftInfo != null) {
              Alert aircraft = Alert.fromAircraft(location, jsonAircraft, aircraftInfo);
              if(Configuration.DEBUG_INJECT_TEST_AIRCRAFTS != 0) {

...
public class DS1ServiceActivity extends AppCompatActivity {
  ...
  protected boolean mServiceOnly = false;
  ...
  private void updateDS1DeviceConnectedText(boolean connected) {
    runOnUiThread(() -> {
      if(connected) {
        mDS1ConnectedImage.setColorFilter(getColor(R.color.status_icon_active));
      }
      else {
        mDS1ConnectedImage.setColorFilter(getColor(R.color.status_icon_inactive));
      }
    });
  }
  ...
  /**
   * Bind to the DS1 service without disconnecting the existing BLE connection.
   * Used by secondary activities (e.g. DS1VolumeActivity) that need the service
   * reference but shouldn't disturb the connection managed by AlertsActivity.
   */
  protected void bindDS1ServiceOnly(Runnable onDone) {
    Log.i(TAG, "bindDS1ServiceOnly");

    // Register a receiver for DS1 data notifications only (not connect/disconnect)
    mDS1Receiver = new BroadcastReceiver() {
      @Override
      public void onReceive(Context context, Intent intent) {
        String action = intent.getAction();
        Log.i(TAG, String.format("bindDS1ServiceOnly onReceive action %s", action));
        if(action.equals(DS1Service.DS1_GOT_RESULT)) {
          onDS1DeviceData();
        }
      }
    };
    ...
    mDS1ServiceConnection = new ServiceConnection() {
      @Override
      public void onServiceConnected(ComponentName name, IBinder s) {
        Log.i(TAG, "bindDS1ServiceOnly.onServiceConnected");
        mDS1Service = ((DS1Service.ThisBinder)s).getService();
        updateDS1DeviceConnectedText(mDS1Service != null && mDS1Service.isConnected());
      ...
      @Override
      public void onServiceDisconnected(ComponentName n) {
        Log.i(TAG, "bindDS1ServiceOnly.onServiceDisconnected");
      }
    };

    Log.i(TAG, "bindService() mDS1ServiceConnection (service-only)");
    Intent ds1ServiceIntent = new Intent(this, ExtendedDS1Service.class);
    bindService(ds1ServiceIntent, mDS1ServiceConnection, BIND_AUTO_CREATE);
  }
  ...
  protected void onDestroy() {
    ...
    if(!mServiceOnly && mDS1Service != null) {
      Log.i(TAG, "mDS1Service.disconnect()");
      mDS1Service.disconnect();
      mDS1Service.close();
    }

...
/**
 * An asynchronous task that fetches Waze reports from a server.
 */
public class ReportsFetchTask implements Runnable {
  ...
  public void run() {
    ...
    if(Configuration.DEBUG_INJECT_TEST_REPORTS != 0) {
      ...
      // Connect to the configured server and fetch Waze reports
      ...
      try {
        ...
        connection.setReadTimeout(Configuration.REPORTS_CONNECT_TIMEOUT);
        connection.connect();

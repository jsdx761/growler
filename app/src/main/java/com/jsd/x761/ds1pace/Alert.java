...
/**
 * Represents an alert: a DS1 alert, a Waze report or an aircraft
 * state vector.
 */
public class Alert {
  private static final String TAG = "ALERT";

  // OpenSky Network state vector array indices
  // See https://openskynetwork.github.io/opensky-api/rest.html#all-state-vectors
  static final int OPENSKY_ICAO24 = 0;
  static final int OPENSKY_CALLSIGN = 1;
  static final int OPENSKY_LONGITUDE = 5;
  static final int OPENSKY_LATITUDE = 6;
  static final int OPENSKY_BARO_ALTITUDE = 7;
  static final int OPENSKY_ON_GROUND = 8;
  static final int OPENSKY_GEO_ALTITUDE = 13;
  ...
  /**
   * Construct a simulated Waze report alert.
   */
  ...
  /**
   * Construct an alert from a JSON object representing a Waze report.
   */
  ...
  public static Alert fromAircraft(Location location, JSONArray jsonAircraft, String[] aircraftInfo) {
    ...
    alert.alertClass = ALERT_CLASS_AIRCRAFT;
    try {
      alert.transponder = jsonAircraft.getString(OPENSKY_ICAO24);
    }
    catch(JSONException e) {
    }
    try {
      alert.callSign = jsonAircraft.getString(OPENSKY_CALLSIGN);
    }
    catch(JSONException e) {
    }
    try {
      alert.onGround = jsonAircraft.getBoolean(OPENSKY_ON_GROUND);
    }
    catch(JSONException e) {
    }
    ...
    try {
      alert.longitude = jsonAircraft.getDouble(OPENSKY_LONGITUDE);
      alert.latitude = jsonAircraft.getDouble(OPENSKY_LATITUDE);
    }
    catch(JSONException e) {
    }
    try {
      alert.altitude = (float)jsonAircraft.getDouble(OPENSKY_GEO_ALTITUDE);
    }
    catch(JSONException e1) {
      try {
        alert.altitude = (float)jsonAircraft.getDouble(OPENSKY_BARO_ALTITUDE);
      }
  ...
  public boolean isSameAircraft(Alert t2) {
    return alertClass == t2.alertClass && transponder != null && transponder.equals(t2.transponder);
  }
  ...
  /**
   * Return a user-friendly display name for a Waze report based on

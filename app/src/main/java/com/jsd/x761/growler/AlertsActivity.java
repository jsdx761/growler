...
public class AlertsActivity extends DS1ServiceActivity {
  ...
  protected void onCreate(Bundle b) {
    ...
    mReportsAPIKey = mSharedPreferences.getString(getString(R.string.key_reports_api_key), getString(R.string.default_reports_api_key));
    try {
    ...
    bindSpeechService(() -> {
      ...
      simulateFilter.addAction("com.jsd.x761.growler.SIMULATE_RADAR");
      simulateFilter.addAction("com.jsd.x761.growler.SIMULATE_RADAR_CLEAR");
      simulateFilter.addAction("com.jsd.x761.growler.SIMULATE_REPORT");
      simulateFilter.addAction("com.jsd.x761.growler.SIMULATE_REPORT_CLEAR");
      simulateFilter.addAction("com.jsd.x761.growler.SIMULATE_AIRCRAFT");
      simulateFilter.addAction("com.jsd.x761.growler.SIMULATE_AIRCRAFT_CLEAR");
      simulateFilter.addAction("com.jsd.x761.growler.SET_VOICE");
      simulateFilter.addAction("com.jsd.x761.growler.SET_PITCH");
      simulateFilter.addAction("com.jsd.x761.growler.SET_RATE");
      simulateFilter.addAction("com.jsd.x761.growler.LIST_VOICES");
      simulateFilter.addAction("com.jsd.x761.growler.LIST_ALL_VOICES");
      simulateFilter.addAction("com.jsd.x761.growler.SYNTHESIZE_TO_FILE");
      simulateFilter.addAction("com.jsd.x761.growler.SYNTHESIZE_FILTERED");
      simulateFilter.addAction("com.jsd.x761.growler.SET_VOICE_BACKEND");
      simulateFilter.addAction("com.jsd.x761.growler.SET_VOICE_MODE");
      simulateFilter.addAction("com.jsd.x761.growler.SET_TUNED_VOICE");
      simulateFilter.addAction("com.jsd.x761.growler.SET_TUNED_PITCH");
      simulateFilter.addAction("com.jsd.x761.growler.SET_TUNED_RATE");
      simulateFilter.addAction("com.jsd.x761.growler.SET_TUNED_FILTER");
      simulateFilter.addAction("com.jsd.x761.growler.SET_VOICE_BY_NAME");
      simulateFilter.addAction("com.jsd.x761.growler.SET_PITCH_SILENT");
      simulateFilter.addAction("com.jsd.x761.growler.SET_RATE_SILENT");
      simulateFilter.addAction("com.jsd.x761.growler.SET_LOCATION");
      mSimulateRadarReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
          ...
          if("com.jsd.x761.growler.SIMULATE_RADAR".equals(action)) {
          ...
          else if("com.jsd.x761.growler.SIMULATE_RADAR_CLEAR".equals(action)) {
            onSimulatedRadarClear();
          }
          else if("com.jsd.x761.growler.SIMULATE_REPORT".equals(action)) {
          ...
          else if("com.jsd.x761.growler.SIMULATE_REPORT_CLEAR".equals(action)) {
            onSimulatedReportClear();
          }
          else if("com.jsd.x761.growler.SIMULATE_AIRCRAFT".equals(action)) {
          ...
          else if("com.jsd.x761.growler.SIMULATE_AIRCRAFT_CLEAR".equals(action)) {
            onSimulatedAircraftClear();
          }
          else if("com.jsd.x761.growler.SET_VOICE".equals(action)) {
            int index = intent.getIntExtra("index", 0);
            mSpeechService.setVoice(index);
          }
          else if("com.jsd.x761.growler.SET_PITCH".equals(action)) {
            float pitch = intent.getFloatExtra("pitch", Configuration.AUDIO_SPEECH_PITCH);
            mSpeechService.setPitch(pitch);
          }
          else if("com.jsd.x761.growler.SET_RATE".equals(action)) {
            float rate = intent.getFloatExtra("rate", Configuration.AUDIO_SPEECH_RATE);
            mSpeechService.setRate(rate);
          }
          else if("com.jsd.x761.growler.LIST_VOICES".equals(action)) {
            mSpeechService.listVoices();
          }
          else if("com.jsd.x761.growler.LIST_ALL_VOICES".equals(action)) {
            mSpeechService.listAllVoices();
          }
          else if("com.jsd.x761.growler.SYNTHESIZE_TO_FILE".equals(action)) {
          ...
          else if("com.jsd.x761.growler.SYNTHESIZE_FILTERED".equals(action)) {
            String text = intent.getStringExtra("text");
            String filename = intent.getStringExtra("filename");
          ...
          else if("com.jsd.x761.growler.SET_VOICE_BACKEND".equals(action)) {
          ...
          else if("com.jsd.x761.growler.SET_VOICE_MODE".equals(action)) {
            int mode = intent.getIntExtra("mode", Configuration.VOICE_MODE_SYSTEM);
            mSpeechService.setVoiceMode(mode);
          }
          else if("com.jsd.x761.growler.SET_TUNED_VOICE".equals(action)) {
          ...
          else if("com.jsd.x761.growler.SET_TUNED_PITCH".equals(action)) {
            float pitch = intent.getFloatExtra("pitch", Configuration.AUDIO_SPEECH_PITCH);
            boolean network = "network".equals(intent.getStringExtra("mode"));
            mSpeechService.setTunedPitch(network, pitch);
          }
          else if("com.jsd.x761.growler.SET_TUNED_RATE".equals(action)) {
            float rate = intent.getFloatExtra("rate", Configuration.AUDIO_SPEECH_RATE);
            boolean network = "network".equals(intent.getStringExtra("mode"));
            mSpeechService.setTunedRate(network, rate);
          }
          else if("com.jsd.x761.growler.SET_TUNED_FILTER".equals(action)) {
          ...
          else if("com.jsd.x761.growler.SET_VOICE_BY_NAME".equals(action)) {
            String voice = intent.getStringExtra("voice");
            if(voice != null) {
              mSpeechService.setVoiceByName(voice);
            }
          }
          else if("com.jsd.x761.growler.SET_PITCH_SILENT".equals(action)) {
            float pitch = intent.getFloatExtra("pitch", Configuration.AUDIO_SPEECH_PITCH);
            mSpeechService.setPitchSilent(pitch);
          }
          else if("com.jsd.x761.growler.SET_RATE_SILENT".equals(action)) {
            float rate = intent.getFloatExtra("rate", Configuration.AUDIO_SPEECH_RATE);
            mSpeechService.setRateSilent(rate);
          }
          else if("com.jsd.x761.growler.SET_LOCATION".equals(action)) {
      ...
      if(mReportsActive != 0 || mAircraftsActive != 0) {
        ...
        if(mUseWakeLock) {
          ...
          mWakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "Growler:LocationWakeLock");
          mWakeLock.acquire();

          // Acquire a WiFi lock to keep the WiFi radio alive
          WifiManager wm = (WifiManager)getSystemService(Context.WIFI_SERVICE);
          mWifiLock = wm.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "Growler:WifiLock");
          mWifiLock.acquire();
        }
        ...
        mOnGetInitialLocationTask = () -> {
          if(mReportsActive != 0) {
            // Regularly fetch crowd-sourced reports. The next poll is
  ...
  protected void onReportsData(List<Alert> reports) {
    if(reports == null) {
      ...
      if(mReportsActive != 0) {
        mReportsActive = 0;
        announceStatus("status_waze_off", String.format("%s alerts are off", mReportsSourceName));
      }
      return;
    }
    else {
      ...
      if(mReportsActive == 0) {
        mReportsActive = 2;
        announceStatus("status_waze_back_on", String.format("%s alerts are back on", mReportsSourceName));
      }
      else if(mReportsActive == 1) {
        mReportsActive = 2;
        announceStatus("status_waze_on", String.format("%s alerts are on", mReportsSourceName));
      }
    }

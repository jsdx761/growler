...
public class SpeechService extends Service {
  ...
  public void onCreate() {
    ...
    mTextToSpeech = new TextToSpeech(getApplicationContext(), status -> {
      if(status != TextToSpeech.ERROR) {
        ...
        float savedSpeed = prefs.getFloat(getString(R.string.key_voice_speed), Configuration.AUDIO_SPEECH_RATE);
        ...
        new Thread(() -> {
          initPreRecordedEngine(segmentsDirForMode(mVoiceMode), savedSpeed);
  ...
  private static final String CHANNEL_ID = "GrowlerLocationChannel";
  private static final int NOTIFICATION_ID = 1;

  public void startForegroundMode() {
    Log.i(TAG, "startForegroundMode");
    NotificationChannel channel = new NotificationChannel(
      CHANNEL_ID, "Growler Location", NotificationManager.IMPORTANCE_LOW);
  ...
  private void checkAllEnginesReady() {
    mHandler.postDelayed(() -> {
      ...
      if(mVoiceMode == Configuration.VOICE_MODE_PRERECORDED && !mPreRecordedInitDone) return;
  ...
  public void setRate(float rate) {
    ...
    if(mPreRecordedEngine != null) {
      mPreRecordedEngine.setSpeed(rate);
    }
    if(usePreRecorded()) {
  ...
  private void initPreRecordedEngine(String segmentsDir, float speed) {
    try {
      mPreRecordedEngine = new PreRecordedTtsEngine();
      mPreRecordedEngine.setSpeed(speed);
  ...
  private void reinitPreRecordedEngine(String segmentsDir) {
    new Thread(() -> {
      ...
      SharedPreferences prefs = getSharedPreferences(
        getString(R.string.preference_file_key), Context.MODE_PRIVATE);
      float speed = prefs.getFloat(getString(R.string.key_voice_speed), Configuration.AUDIO_SPEECH_RATE);
      initPreRecordedEngine(segmentsDir, speed);
    }).start();
  }

...
app's assets/voice_segments_voxtral_NNN/ directories.  Generates a
segments_index.txt manifest listing all available segment IDs.

Packages the base segments (speed 1.00 → voice_segments_voxtral_100)
plus any speed variants listed in voxtral_speed.speeds.
...
def run(cfg: dict):
    ...
    # Package all speeds from config (includes 1.00 base + variants)
    speeds = cfg.get("voxtral_speed", {}).get("speeds", [1.0])

    log.info(f"Packaging {len(speeds)} Voxtral speed variant(s): {speeds}")

    for speed in speeds:
        spd = int(speed * 100)
        src_dir = output_dir / f"voxtral_segments_{candidates}c_spd_{spd:03d}"

        if not src_dir.exists():
            log.error(f"Segments directory not found: {src_dir}")
            if speed == 1.0:
                log.error("Run stage 2 (voxtral_segments) first.")
            else:
                log.error("Run stage 3 (voxtral_speed) first.")
            continue

        assets_dir = assets_base / f"voice_segments_voxtral_{spd:03d}"
        package_set(src_dir, assets_dir)

    log.info("All configured Voxtral segment sets packaged.")
